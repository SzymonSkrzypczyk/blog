...
export default function AboutRightSection({ posts, activeSection }: Props) {
    return (
        <div className="flex flex-col justify-start bg-backgroundColor lg:pl-10 lg:py-10 lg:gap-32 px-5 mx-auto gap-16">
            <div id="about">
                <h2 className={`m-0 w-full text-textColor font-bold text-2xl pb-4 lg:hidden ${activeSection == "#about" ? "sticky top-0 backdrop-blur-sm" : ""}`}>About</h2>
                <AboutRightSectionText />
            </div>
            <div id="experience" className="flex flex-col lg:gap-3 gap-4">
                <h2 className={`text-textColor font-bold text-2xl pb-4 lg:hidden ${activeSection == "#experience" ? "sticky top-0 backdrop-blur-sm" : ""}`}>Experience</h2>
            ...
            <div id="projects" className="flex flex-col lg:gap-3 gap-6">
                <h2 className={`text-textColor font-bold text-2xl pb-4 lg:hidden ${activeSection == "#projects" ? "sticky top-0 backdrop-blur-sm" : ""}`}>Projects</h2>
                ...
                <div className="flex flex-row lg:pl-2 lg:ml-2 group gap-2 align-baseline">
                    <a href="https://github.com/SzymonSkrzypczyk?tab=repositories" className="text-textColor font-bold text-xl">More projects</a>
            ...
            <div id="blog" className="lg:mb-8 mb-16">
                <h2 className={`text-textColor font-bold text-2xl pb-4 lg:hidden ${activeSection == "#blog" ? "sticky top-0 backdrop-blur-sm" : ""}`}>Blog</h2>
                {posts.map((post) => (
                    <AboutRightSectionPost key={post.id} post={post} />
                ))}
                <div className="flex flex-row lg:pl-2 lg:ml-2 group gap-2 align-baseline pb-50 lg:pt-10">
                    <a href="/posts" className="text-textColor font-bold text-xl">More posts</a>
                    <img src="storage/images/link-arrow.svg" alt="arrow" className="group-hover:animate-ping duration-700 transition-all w-4"/>
                </div>
            </div>
        </div>
